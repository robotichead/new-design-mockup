...
const defineClass = computed(() => {
    ...
    isActive = isActive || route?.path?.toLowerCase() === props.route.toLowerCase();

    // If the route is the same as the props routeNew - we are currently on that route
    isActive = isActive || route?.path?.toLowerCase() === props.routeNew.toLowerCase();
