...
const menuIconSize = computed(() => {
    return window.innerWidth < 640 ? 36 : 16;
});

const toggleMenuText = computed(() => {
    return navStore.isNavOpen ? 'Close Navigation Menu' : 'Open Navigation Menu';
});
...
<template>
    <header role="banner">
        ...
        <div
            class="nearbeach-logo"
            aria-hidden="true"></div>
        <div class="header--profile">
            <img
                src="../../assets/Morgana.jpeg"
                alt="Morgana" />
        </div>
    </header>
</template>
